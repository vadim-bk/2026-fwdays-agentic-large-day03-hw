...
**Input**: Optionally specify a change name. If omitted, infer from conversation context when the active change is unambiguous; if vague, ambiguous, or several changes could apply, you MUST use **AskUserQuestion** (after `openspec list --json`) so the user picks.
...
4. **Check task completion status**
   ...
   For markdown checkbox lists, count `- [ ]` (incomplete) vs `- [x]` (complete). For other formats, use schema-appropriate markers only if applicable; if the file is not a checkbox list, skip counting and proceed without this warning unless the schema defines another rule.
...
5. **Assess delta spec sync state**
   ...
   **Branch explicitly:**
   ...
   - **"Archive now"** (already-synced path): perform **step 6** immediately—do **not** invoke the Task tool or `openspec-sync-specs`; go straight to the archive routine in this skill.
